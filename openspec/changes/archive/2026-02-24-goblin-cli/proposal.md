...
We need a repeatable CLI pipeline that turns RSS blog posts into structured, LLM-ready artifacts and stores them for search and retrieval. Building this now unlocks consistent ingest/extract/store workflows and eliminates ad-hoc scripts.

## What Changes

- Add a new `@grogbot/goblin` CLI package under `packages/goblin/` (ESM) with a bin entry.
- Implement `ingest`, `extract`, and `store` subcommands to support a full RSS → markdown → LLM extract → chunked storage pipeline.
...
### New Capabilities
- `goblin-cli`: CLI tooling for RSS ingest, LLM extract processing, and Upstash Search storage.
