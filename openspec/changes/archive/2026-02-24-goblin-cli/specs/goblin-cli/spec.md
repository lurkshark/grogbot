...
The `extract` subcommand SHALL read an RSS feed URL and a pond directory, then write one markdown file per valid feed item into `pond/ingest/`.

#### Scenario: Extract persists items to pond
- **WHEN** the user runs `goblin extract <feed-url> <pond>` with a feed containing valid items
...
- **WHEN** the user runs `goblin transform <prompt> <namespace> <pond>`
