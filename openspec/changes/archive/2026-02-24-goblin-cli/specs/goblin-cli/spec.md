...
The `extract` subcommand SHALL read an RSS feed URL and a staging directory, then write one markdown file per valid feed item into `staging-directory/ingest/`.

#### Scenario: Extract persists items to staging directory
- **WHEN** the user runs `goblin extract <feed-url> <staging-directory>` with a feed containing valid items
...
- **WHEN** the user runs `goblin transform <prompt> <namespace> <staging-directory>`
